 ...
 * Rozszerzone o migrację danych kursu i bezpieczną walidację zapisu 6.6.0.
 */
...
function validateImportedState(rawText){
  ...
  const allowedTop=new Set(['version','contentVersion','createdAt','settings','user','items','wordProgress','days','daily','mistakes','modeStats','sessions','activeSession','recentWordIds','course']);
  ...
  string(parsed.contentVersion,'contentVersion',120);
  ...
  const validModes=new Set(['course_intro','course_intro_group','course_grammar','course_final','course','word_choice','word_write','sentence_translate','listening_write','speaker_repeat','speaking','dialogue','car_voice','en_pl','mixed','test','reviews','vocab','sentences','writing','listening','lector','car','dialogues']);
  ...
  const validateTask=(task,name)=>{assertObject(task,name);assertKnown(task,new Set(['taskId','wordId','materialIds','choicePoolIds','kind','mode','stage','required','counted','prompt','hint','answerMode','acceptedAnswers','expectedAnswer','exampleAnswer','minimumWords','requiredKeywords','minimumKeywordGroups','requiredChecks','minimumSentences','minimumKeywordSentences','minimumPeople','actionGroups','minimumActions','grammarNote']),name);if(task.wordId!==undefined&&task.wordId!==null&&!validIds.has(task.wordId))throw new Error('Nieznany materiał w '+name+'.');if(task.materialIds!==undefined&&(!Array.isArray(task.materialIds)||task.materialIds.length>20||task.materialIds.some(id=>typeof id!=='string'||!validIds.has(id))))throw new Error('Nieprawidłowa grupa materiałów w '+name+'.');if(task.choicePoolIds!==undefined&&(!Array.isArray(task.choicePoolIds)||task.choicePoolIds.length>30||task.choicePoolIds.some(id=>typeof id!=='string'||!validIds.has(id))))throw new Error('Nieprawidłowa pula odpowiedzi w '+name+'.');if(!task.wordId&&!task.materialIds?.length&&!['course_final','course_grammar'].includes(task.mode))throw new Error('Brak materiału w '+name+'.');if(!['new','review','weak'].includes(task.kind))throw new Error('Nieprawidłowy rodzaj zadania w '+name+'.');if(!validModes.has(task.mode))throw new Error('Nieprawidłowy tryb zadania w '+name+'.');if(task.stage!==undefined&&!CourseCore.ALLOWED_STAGES.includes(task.stage))throw new Error('Nieprawidłowy etap zadania w '+name+'.');bool(task.required,name+'.required');bool(task.counted,name+'.counted');string(task.taskId,name+'.taskId',300);string(task.prompt,name+'.prompt',1000);string(task.hint,name+'.hint',1000);string(task.expectedAnswer,name+'.expectedAnswer',1000);string(task.exampleAnswer,name+'.exampleAnswer',1000);number(task.minimumWords,0,100,name+'.minimumWords');number(task.minimumKeywordGroups,0,100,name+'.minimumKeywordGroups');number(task.minimumSentences,0,20,name+'.minimumSentences');number(task.minimumKeywordSentences,0,20,name+'.minimumKeywordSentences');number(task.minimumPeople,0,20,name+'.minimumPeople');number(task.minimumActions,0,20,name+'.minimumActions');if(task.acceptedAnswers!==undefined&&(!Array.isArray(task.acceptedAnswers)||task.acceptedAnswers.length>50||task.acceptedAnswers.some(value=>typeof value!=='string'||value.length>1000)))throw new Error('Nieprawidłowe warianty odpowiedzi w '+name+'.');if(task.grammarNote!==undefined&&(!task.grammarNote||typeof task.grammarNote!=='object'||typeof task.grammarNote.title!=='string'||typeof task.grammarNote.explanation!=='string'))throw new Error('Nieprawidłowa notatka gramatyczna w '+name+'.');if(task.requiredKeywords!==undefined&&(!Array.isArray(task.requiredKeywords)||task.requiredKeywords.length>50||task.requiredKeywords.some(group=>!Array.isArray(group)||!group.length||group.some(value=>typeof value!=='string'||value.length>200))))throw new Error('Nieprawidłowe kryteria słów kluczowych w '+name+'.');if(task.requiredChecks!==undefined&&(!Array.isArray(task.requiredChecks)||task.requiredChecks.length>20||task.requiredChecks.some(value=>typeof value!=='string'||!['self_name'].includes(value))))throw new Error('Nieprawidłowe kryteria znaczeniowe w '+name+'.');if(task.actionGroups!==undefined&&(!Array.isArray(task.actionGroups)||task.actionGroups.length>30||task.actionGroups.some(group=>!Array.isArray(group)||!group.length||group.some(value=>typeof value!=='string'||value.length>200))))throw new Error('Nieprawidłowe grupy czynności w '+name+'.');};
  const validSources=new Set(['practice','review','course','car']);
  const resultFields=new Set(['taskId','wordId','materialIds','stage','mode','status','score','counted','required','completed','answeredAt']);
  const validateTaskResults=(values,name)=>{if(values===undefined)return;if(!Array.isArray(values)||values.length>500)throw new Error('Nieprawidłowe wyniki zadań w '+name+'.');values.forEach((entry,index)=>{assertObject(entry,`${name}[${index}]`);assertKnown(entry,resultFields,`${name}[${index}]`);string(entry.taskId,`${name}[${index}].taskId`,300);if(entry.wordId!==undefined&&entry.wordId!==null&&!validIds.has(entry.wordId))throw new Error('Nieznany materiał w wynikach zadania.');if(entry.materialIds!==undefined&&(!Array.isArray(entry.materialIds)||entry.materialIds.some(id=>typeof id!=='string'||!validIds.has(id))))throw new Error('Nieznana grupa materiałów w wynikach zadania.');if(entry.stage!==undefined&&!CourseCore.ALLOWED_STAGES.includes(entry.stage))throw new Error('Nieznany etap w wynikach zadania.');if(entry.mode!==undefined&&!validModes.has(entry.mode))throw new Error('Nieznany tryb w wynikach zadania.');if(entry.status!==undefined&&!['correct','partial','wrong','completed'].includes(entry.status))throw new Error('Nieznany status wyniku zadania.');number(entry.score,0,1,`${name}[${index}].score`,false);bool(entry.counted,`${name}[${index}].counted`);bool(entry.required,`${name}[${index}].required`);bool(entry.completed,`${name}[${index}].completed`);isoDate(entry.answeredAt,`${name}[${index}].answeredAt`);});};
  if(parsed.sessions!==undefined){if(!Array.isArray(parsed.sessions)||parsed.sessions.length>500)throw new Error('Nieprawidłowa historia sesji.');parsed.sessions.forEach((entry,index)=>{assertObject(entry,'sessions['+index+']');assertKnown(entry,new Set(['date','practice','source','lessonId','originLessonId','moduleId','lessonTitle','correct','wrong','xp','total','courseResult','courseAccuracy','finalTaskCompleted']),'sessions['+index+']');isoDate(entry.date,`sessions[${index}].date`);if(typeof entry.practice!=='string'||!validModes.has(entry.practice))throw new Error('Nieprawidłowy tryb sesji.');if(entry.source!==undefined&&!validSources.has(entry.source))throw new Error('Nieprawidłowe źródło sesji.');for(const f of ['correct','wrong','xp','total'])number(entry[f],0,1000000,`sessions[${index}].${f}`);number(entry.courseAccuracy,0,100,`sessions[${index}].courseAccuracy`);bool(entry.finalTaskCompleted,`sessions[${index}].finalTaskCompleted`);if(entry.total!==undefined&&entry.correct!==undefined&&entry.wrong!==undefined&&entry.correct+entry.wrong>entry.total)throw new Error('Niespójny wynik sesji.');});}
  if(parsed.activeSession!==undefined&&parsed.activeSession!==null){const a=parsed.activeSession;assertObject(a,'activeSession');assertKnown(a,new Set(['queue','index','correct','wrong','xp','practice','source','startedAt','lessonId','originLessonId','moduleId','lessonTitle','lessonGoal','taskResults','stageProgress','finalTaskCompleted']),'activeSession');if(!Array.isArray(a.queue)||!a.queue.length||a.queue.length>500)throw new Error('Nieprawidłowa kolejka aktywnej sesji.');a.queue.forEach((task,index)=>validateTask(task,`activeSession.queue[${index}]`));number(a.index,0,a.queue.length,'activeSession.index');for(const f of ['correct','wrong','xp'])number(a[f],0,1000000,'activeSession.'+f);if(typeof a.practice!=='string'||!validModes.has(a.practice))throw new Error('Nieprawidłowy tryb aktywnej sesji.');if(a.source!==undefined&&!validSources.has(a.source))throw new Error('Nieprawidłowe źródło aktywnej sesji.');isoDate(a.startedAt,'activeSession.startedAt');validateTaskResults(a.taskResults,'activeSession.taskResults');assertObject(a.stageProgress,'activeSession.stageProgress');bool(a.finalTaskCompleted,'activeSession.finalTaskCompleted');}
