...
function practiceQueue(kind){
  ...
  if(kind==='car'){
    const seenKeys=new Set(),known=activeWords().filter(word=>prog(word.id).seen>0&&sentenceEn(word).includes(' '));
    const priority=[...orderedReviews(due().filter(word=>known.some(item=>item.id===word.id))),...weak().filter(word=>known.some(item=>item.id===word.id)),...preferNotRecent(known)];
    ...
    return selected.slice(0,target).map(word=>task(word,prog(word.id).status==='weak'?'weak':'review','car_voice'));
  }
  if(kind==='dialogues'){
