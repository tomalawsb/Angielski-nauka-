...
function completeCourseInfoTask(){
  const current=curTask();if(!current||current.mode!=='course_intro'||!session||session.source!=='course')return false;
  recordCourseTaskResult(current,{status:'completed',score:1});markCourseMaterialIntroduced(current.wordId);return true;
}
