...
function courseLessonRow(snapshot,lesson){
  ...
  const accuracy=progress.bestAccuracy?` · najlepszy wynik ${progress.bestAccuracy}%`:'';
  return `<button class="course-lesson-row ${canStart?'is-clickable':''}" data-course-lesson="${esc(lesson.id)}" ${canStart?'':'disabled'}><span class="course-lesson-number">${lesson.order}</span><span class="course-lesson-copy"><strong>${esc(lesson.title)}</strong><small>${esc(lesson.goal)}</small>${accuracy?`<small>${esc(accuracy.slice(3))}</small>`:''}</span><span class="course-status ${courseStatusClass(status)}">${esc(courseStatusText(status))}</span></button>`;
}
